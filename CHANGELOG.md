...
## Unreleased — quality-first creative relay

- 新默认采用独立导演 → 新的连续章节制作上下文 → 独立审美 → 原制作角色定点返修。质量优先，删除单一创作负责人默认，不以节省 Token 为由跳过审美或必要修改。
- 新增 `prepare-creative-task.mjs`：按角色生成单份精简交接任务，带实际绝对路径、完整原始输入、本章与相邻接缝；提前指出缺失资料。它不启动模型、不改变宿主权限，也不宣称自动清除全局上下文。
- Parent 集中运行计划校验、渲染、解码、缓存与拼接；制作角色处理画面问题。独立审美只接收实际媒体和需求，不接收制作过程、代码或成本；同时有 draft/final 时明确选择本次审看的版本。
- 新增三种按语义选择的紧凑动效参考及原创、可执行的 8 秒示例，落实层次、动作因果、停稳和跨镜继承；不强制固定配色、纸片模板或元素数量，不一刀切禁用 SVG。
- 保留 `lean-1` 计划、全局帧边界、draft/final 分离、逐镜缓存和失败恢复。已有 Recipe/runtime-plan v1–v4、Remotion/hybrid 继续兼容入口。
- 32 秒同输入 Sol 盲评偏向接力样片 Y，用户随后明确采用 Y、效果优先。实际单镜改色后 8.569 秒更新，其他三镜哈希不变；尚未证明节省端到端时间或 Token，保留失败和环境干扰记录。
- 五语 README 同步新流程；测试样片不在首页展示。main 源码更新与 v1.0.1 版本化 Release 区分，不移动旧 tag 或覆盖旧安装包。
