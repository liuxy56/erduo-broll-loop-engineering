...
v1.1.0 同时提供标准 Skill 包和完整环境安装包，两包采用同一份质量优先接力流程。
发布时核对版本、五语安装链接、两包 Skill 内容、安装注册和 GitHub CI；
从公开 Release 下载后再次核对归档摘要与实际 Skill 内容。
原首页展示、流程图和作者联系方式保留，Y 测试样片不进入首页或安装包。

生产与审美证据见 [当前验证记录](docs/LEAN-WORKFLOW-VALIDATION.md)。
32 秒对照与 8 秒示例不能外推为长片保证，也尚未证明整体时间或 Token 节省。
下面保留 v1.0.1 及更早版本的检查记录，不将历史勾选当作 v1.1.0 新增实测。
